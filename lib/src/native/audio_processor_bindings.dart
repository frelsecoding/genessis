...
/// Bindings for native audio processing functions (Mel filterbank).
class AudioProcessorBindings {
  /// Holds the symbol lookup function.
  final ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName)
      _lookup;

  /// The symbols are looked up in [dynamicLibrary].
  AudioProcessorBindings(ffi.DynamicLibrary dynamicLibrary)
      : _lookup = dynamicLibrary.lookup;

  /// The symbols are looked up with [lookup].
  AudioProcessorBindings.fromLookup(
      ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName)
          lookup)
      : _lookup = lookup;

  /// @brief Calculates a Mel filterbank.
  ///
  /// The caller is responsible for freeing the returned float pointer using
  /// free_mel_filterbank_memory.
  /// The memory for out_rows and out_cols must be allocated by the caller
  /// and will be populated by this function.
  ///
  /// @param n_fft The number of FFT components.
  /// @param n_mels_in The number of Mel bands to generate.
  /// @param sr_in The sampling rate of the incoming signal.
  /// @param f_min_in The lowest frequency (in Hz).
  /// @param f_max_in The highest frequency (in Hz). If <= 0, sr_in / 2.0 will be used.
  /// @param out_rows Pointer to an int32_t where the number of rows (n_mels_in) will be stored.
  /// @param out_cols Pointer to an int32_t where the number of columns (1 + n_fft / 2) will be stored.
  /// @return A pointer to the calculated Mel filterbank (row-major order), or NULL on failure.
  ffi.Pointer<ffi.Float> calculate_mel_filterbank(
    int n_fft,
    int n_mels_in,
    double sr_in,
    double f_min_in,
    double f_max_in,
    ffi.Pointer<ffi.Int32> out_rows,
    ffi.Pointer<ffi.Int32> out_cols,
  ) {
    return _calculate_mel_filterbank(
      n_fft,
      n_mels_in,
      sr_in,
      f_min_in,
  ...
  late final _calculate_mel_filterbankPtr = _lookup<
      ffi.NativeFunction<
          ffi.Pointer<ffi.Float> Function(
              ...
              ffi.Pointer<ffi.Int32>)>>('calculate_mel_filterbank');
  late final _calculate_mel_filterbank =
      _calculate_mel_filterbankPtr.asFunction<
          ffi.Pointer<ffi.Float> Function(int, int, double, double, double,
              ffi.Pointer<ffi.Int32>, ffi.Pointer<ffi.Int32>)>();

  /// @brief Frees the memory allocated for a Mel filterbank by calculate_mel_filterbank.
  ///
  /// @param ptr Pointer to the memory to be freed.
  void free_mel_filterbank_memory(
    ffi.Pointer<ffi.Void> ptr,
  ) {
    return _free_mel_filterbank_memory(
      ptr,
    );
  }

  late final _free_mel_filterbank_memoryPtr =
      _lookup<ffi.NativeFunction<ffi.Void Function(ffi.Pointer<ffi.Void>)>>(
          'free_mel_filterbank_memory');
  late final _free_mel_filterbank_memory = _free_mel_filterbank_memoryPtr
